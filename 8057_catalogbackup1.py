...
class Hannas_Handiworks():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			self.prod[sku]['sale'] = float(rsheet.row(x)[13].value) if rsheet.row(x)[10].value == rsheet.row(x)[12].value  else ""
			self.prod[sku]['set'] = ""
			self.prod[sku]['custom'] = ""
			self.prod[sku]['size'] = rsheet.row(x)[8].value
			self.prod[sku]['top'] = ""
			self.prod[sku]['min'] = float(rsheet.row(x)[10].value) if rsheet.row(x)[10].value != "NA" else float(1)
			self.prod[sku]['price1'] = float(rsheet.row(x)[11].value)
			self.prod[sku]['min2'] = rsheet.row(x)[12].value if rsheet.row(x)[10].value != rsheet.row(x)[12].value  else ""
			self.prod[sku]['price2'] = rsheet.row(x)[13].value if rsheet.row(x)[10].value != rsheet.row(x)[12].value  else ""
			self.prod[sku]['min3'] = float(rsheet.row(x)[14].value) if rsheet.row(x)[14].value != "" else ""
			self.prod[sku]['price3'] = float(rsheet.row(x)[15].value) if rsheet.row(x)[15].value != "" else ""
			self.prod[sku]['multi'] = float(rsheet.row(x)[16].value) if rsheet.row(x)[16].value != "NA" else 1
