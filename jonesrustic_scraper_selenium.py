...
br.get("https://www.jonesrusticsigns.com/catalog/seo_sitemap/product/")
time.sleep(1)

def get_items():
	while True:
		try:
			item = [a.get_attribute("href") for a in br.find_elements_by_css_selector("body > div.wrapper > div > div.main-container.col1-layout.content-color.color > div > div > div > div > div > ul.sitemap > li > a")]
			print item
			items.extend(item)
			time.sleep(1)
			break
...
def nextPage():
	try:
		br.get(br.find_element_by_css_selector("body > div.wrapper > div > div.main-container.col1-layout.content-color.color > div > div > div > div > div > div:nth-child(3) > div > ol > li.bor-none > a.next").get_attribute("href"))
		time.sleep(1)
		print "Next page."
		time.sleep(3)
		return True
