...
class Designs_Combined():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			self.prod[sku]['stock'] = ""
			self.prod[sku]['sale'] = ""
			self.prod[sku]['set'] = rsheet.row(x)[2].value.split("List Price:")[1].split()[5].split("/")[1] if "s/" in rsheet.row(x)[2].value.split("List Price:")[1].split()[5] else ""
			...
			try:
				...
				self.prod[sku]['price2'] = float(rsheet.row(x)[2].value.split("List Price:")[1].split()[6].strip("$"))
			except:
				self.prod[sku]['min2'] = ""
				self.prod[sku]['price2'] = ""
			try:
				self.prod[sku]['min3'] = float(rsheet.row(x)[2].value.split("List Price:")[1].split()[12])
				self.prod[sku]['price3'] = float(rsheet.row(x)[2].value.split("List Price:")[1].split()[10].strip("$"))
			except:
				self.prod[sku]['min3'] = ""
