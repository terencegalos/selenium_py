...
from helper.bhbt_class import BHBT
...
def updateBHBT(vendor,mode=None):
	...
	local = BHBT(vendor,3) #  shst instance of shst file
	# print local
	...
	for key in wares.prod:
		if key not in local.prod: # add new
			...
			#  local.prod[key]['size'] == wares.prod[key]['size'] and \

			#  local.prod[key]['name'] == wares.prod[key]['name'] and \
			#  local.prod[key]['jpg800'] == wares.prod[key]['jpg800'] and \
			if local.prod[key]['price1'] == wares.prod[key]['price1'] and \
			 ...
			 local.prod[key]['stock'].lower() == wares.prod[key]['stock'].lower():
...
def makeXLS(wobj,vendor):
	wbook = xls()
	# wsheet = wbook.create_sheet("Sheet1",0)
	wsheet = wbook.add_sheet("Sheet1")

	for count,i in enumerate(wobj.prod):
		row = wsheet.row(count)
		val = [wobj.prod[i]['name'],wobj.prod[i]['sku'],wobj.prod[i]['desc'],wobj.prod[i]['stock'],wobj.prod[i]['size'],wobj.prod[i]['min'],wobj.prod[i]['price1'],wobj.prod[i]['min2'],wobj.prod[i]['price2'],wobj.prod[i]['min3'],wobj.prod[i]['price3'],wobj.prod[i]['multi'],wobj.prod[i]['jpg800'],wobj.prod[i]['qty'],wobj.prod[i]['msrp'],wobj.prod[i]['sale'],wobj.prod[i]['ship'],wobj.prod[i]['isactive'],wobj.prod[i]['brand'],wobj.prod[i]['category'],wobj.prod[i]['opt'],wobj.prod[i]['section'],wobj.prod[i]['desc2'],wobj.prod[i]['brandCat'],wobj.prod[i]['space'],wobj.prod[i]['brandSec'],wobj.prod[i]['isUpdateAvailable']]
