...
class Ragon_House_Collection():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			# print rsheet.row(x)
			if rsheet.row(x)[6].value == '': # skip no price
				continue
			...
			try:
				...
				sku = str(int(rsheet.row(x)[0].value)) # convert to int to remove decimals
			except ValueError:
				sku = "".join(rsheet.row(x)[0].value.split())
			except Exception as e:
				raise(f"Error in sku:{rsheet.row(x)[0].value} {e}")
			...
			self.prod[sku]['size'] = str(rsheet.row(x)[11].value) +"l x "+ str(rsheet.row(x)[12].value) +"w x "+ str(rsheet.row(x)[13].value)+"h"
			self.prod[sku]['top'] = ""#rsheet.row(x)[9].value
			self.prod[sku]['min'] = round(rsheet.row(x)[3].value,2)# if self.is_num(rsheet.row(x)[3].value) else rsheet.row(x)[10].value
			self.prod[sku]['price1'] = round(float(rsheet.row(x)[6].value),2)
			...
			self.prod[sku]['multi'] = round(rsheet.row(x)[3].value,2)# if self.is_num(rsheet.row(x)[16].value) else rsheet.row(x)[16].value
