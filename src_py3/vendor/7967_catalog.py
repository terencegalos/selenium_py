...
class Ragon_House_Collection():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			# print rsheet.row(x)
			# if rsheet.row(x)[3].value == '': # skip no price
			# 	continue
			...
			try:
				...
				sku = str(int(rsheet.row(x)[1].value)) # convert to int to remove decimals
			except ValueError:
				sku = "".join(rsheet.row(x)[1].value.split())
			except Exception as e:
				raise(f"Error in sku:{rsheet.row(x)[1].value} {e}")
			# 	sku = "".join(rsheet.row(x)[1].value.split())
			...
			self.prod[sku]['stock'] = "In-Stock: Ships in 1 to 3 days" if rsheet.row(x)[6].value > 3 else "Out of stock"
			self.prod[sku]['sale'] = ""#rsheet.row(x)[7].value if float(rsheet.row(x)[6].value) != float(rsheet.row(x)[7].value) else ""
			self.prod[sku]['set'] = ""#rsheet.row(x)[6].value
			self.prod[sku]['custom'] = ""#rsheet.row(x)[7].value
			self.prod[sku]['size'] = str(rsheet.row(x)[12].value) +"l x "+ str(rsheet.row(x)[13].value) +"w x "+ str(rsheet.row(x)[14].value)+"h"
			self.prod[sku]['top'] = ""#rsheet.row(x)[9].value
			self.prod[sku]['min'] = round(rsheet.row(x)[7].value,2)# if self.is_num(rsheet.row(x)[3].value) else rsheet.row(x)[10].value
			...
			self.prod[sku]['multi'] = round(rsheet.row(x)[7].value,2)# if self.is_num(rsheet.row(x)[16].value) else rsheet.row(x)[16].value
			self.prod[sku]['img400'] = "Ragon400"
			self.prod[sku]['img160'] = "Ragon160"
			self.prod[sku]['jpg400'] = rsheet.row(x)[8].value
			self.prod[sku]['jpg160'] = rsheet.row(x)[8].value.split("/")[-1]
			self.prod[sku]['desc2'] = ""#rsheet.row(x)[9].value
			self.prod[sku]['opt'] = ""#rsheet.row(x)[22].value
			self.prod[sku]['img800'] = "Ragon800"
			self.prod[sku]['jpg800'] = rsheet.row(x)[8].value.split("/")[-1]
