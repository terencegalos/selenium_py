...
class wingtai(domainobject):
    ...
    def init_login(self,un,pw):
        self.driver.get(self.home)
        self.time.sleep(2)
        self.driver.find_element(By.CSS_SELECTOR,"#pre-login-navbar > li.signIn-li > a").click()
        self.time.sleep(1)
        # self.driver.find_element(By.CSS_SELECTOR,"#regPullLogin").click()
        # self.time.sleep(1)
    ...
    def get_info(self,sku=None):
        ...
        db = gateway()
        try:
            db.name = self.driver.find_element(By.CSS_SELECTOR,"#moredesc").text.encode("utf-8")
            print(db.name)
        except:
        ...
        try:
            db.sku = self.driver.find_element(By.CSS_SELECTOR,"#moreside > h1").text.encode("utf-8")
        except:
            # print self.driver.get_attribute("innerHTML")
            print("Skipping. Possible noise.")
        ...
        try:
            db.stock = self.driver.find_element(By.CSS_SELECTOR,"div > span").text.encode("utf-8")
        except:
            db.stock = ""

        try:
            db.sale = self.driver.find_element(By.CSS_SELECTOR,"#bb-mioptt").text.encode("utf-8")
        except:
            db.sale = ""
        db.set = ""
        db.custom = ""
        ...
        try:
            db.min1 = self.driver.find_element(By.CSS_SELECTOR,"td.qp_quantity:nth-child(2)").text.encode("utf-8")
            if "-" in db.min1:
                db.min1 = db.min1.split("-")[0].strip()
            db.price1 = self.driver.find_element(By.CSS_SELECTOR,"td.qp_price:nth-child(2)").text.encode("utf-8").strip("$")
        ...
        try:
            db.min2 = self.driver.find_element(By.CSS_SELECTOR,"td.qp_quantity:nth-child(3)").text.encode("utf-8")
            if "+" in db.min2:
                db.min2 = db.min2.strip("+").strip()
            if "-" in db.min2:
                db.min2 = db.min2.split("-")[0].strip()
            db.price2 = self.driver.find_element(By.CSS_SELECTOR,"td.qp_price:nth-child(3)").text.encode("utf-8").strip("$")
        ...
        try:
            db.min3 = self.driver.find_element(By.CSS_SELECTOR,"td.qp_quantity:nth-child(4)").text.encode("utf-8").strip("+")
            if "-" in db.min3:
                db.min3 = db.min3.split("-")[0].strip().strip("+")
            if "+" in db.min3:
                db.min3 = db.min3.strip().strip("+")
            db.price3 = self.driver.find_element(By.CSS_SELECTOR,"td.qp_price:nth-child(4)").text.encode("utf-8").strip("$")
        ...
        try:
            # db.img400 = self.driver.find_element(By.CSS_SELECTOR,"#moreimage > div > img").get_attribute("src")
            db.img400 = self.driver.find_element(By.CSS_SELECTOR,"#bb-miimghalf > div > img").get_attribute("src")
