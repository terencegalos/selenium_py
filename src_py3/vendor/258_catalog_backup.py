...
class Capitol_Imports():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			self.prod[sku]['top'] = ""
			self.prod[sku]['min'] = float(rsheet.row(x)[9].value)
			self.prod[sku]['price1'] = ound(float(rsheet.row(x)[10].value),2) if self.is_num(rsheet.row(x)[10].value) else rsheet.row(x)[10].value
			self.prod[sku]['min2'] = ""
