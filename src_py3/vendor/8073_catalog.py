...
class JanMichaels_Home_Closeout_Sale():
	prod = {}
	skus = None
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			# print rsheet.row(x)
			if rsheet.row(x)[2].value == "": #skip no sku
				continue
			...
			if isinstance(rsheet.row(x)[7].value,float):
				self.prod[sku]['price1'] = round(float(rsheet.row(x)[7].value/.77),2) if isinstance(rsheet.row(x)[7].value,float) else round(float(rsheet.row(x)[7].value.split("]")[1]/.77),2)
			else:
				# print rsheet.row(x)[6].value.split("]")
				self.prod[sku]['price1'] = round(float(rsheet.row(x)[7].value.split("]")[1]),2) / .77
