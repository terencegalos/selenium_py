...
class Adams_Co_Closeouts():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			if "disc" not in rsheet.row(x)[11].value.lower(): #select only 50 off
				continue
			...
			self.prod[sku]['price2'] = (rsheet.row(x)[6].value * 2 *.9) if rsheet.row(x)[6].value != rsheet.row(x)[3].value else ""
			self.prod[sku]['min3'] = ""
