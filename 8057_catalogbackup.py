...
class Hannas_Handiworks():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			self.prod[sku]['top'] = ""
			self.prod[sku]['min'] = rsheet.row(x)[3].value.split("/")[0]
			self.prod[sku]['price1'] = rsheet.row(x)[3].value.split("/")[1]
			self.prod[sku]['min2'] = rsheet.row(x)[4].value.split("/")[0]
			self.prod[sku]['price2'] = rsheet.row(x)[4].value.split("/")[1]
			self.prod[sku]['min3'] = rsheet.row(x)[5].value.split("/")[0]
			self.prod[sku]['price3'] = rsheet.row(x)[5].value.split("/"[1]
			self.prod[sku]['multi'] = rsheet.row(x)[3].value.split("/")[0]
