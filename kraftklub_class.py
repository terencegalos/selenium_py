...
class kraftklub(domainobject.domainobject):

    vendor = "Kraft Klub"
    url = "http://kraftklub.com/myaccount.php"
    login = "https://kraftklub.com/customer/login"
    home = "http://kraftklub.com/index.html"
    lastStop = "https://kraftklub.com/white-cocktail-napkin-holder-6-5-lx6-5-wx2-h"
    flag = False
    ...
    def get_info(self,item=None):
		...
		# db.name = ""
		try:
			db.name = self.driver.find_element_by_css_selector("#product-form > div > div.right.col-lg-7 > div.row.info > h2:nth-child(2)").text.encode("utf-8")
		except:
			return
		db.sku = self.driver.find_element_by_css_selector("#product-form > div > div.right.col-lg-7 > div.row.info > h2:nth-child(1)").text.split()[0].encode("utf-8")
