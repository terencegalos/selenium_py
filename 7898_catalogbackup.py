...
class KMI_International():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			self.prod[sku]['desc'] = ""
			self.prod[sku]['stock'] = ""
			self.prod[sku]['sale'] = "" #float(rsheet.row(x)[18].value) if self.is_num(rsheet.row(x)[18].value) else rsheet.row(x)[18].value
			self.prod[sku]['set'] = ""
			...
			self.prod[sku]['price2'] = float(rsheet.row(x)[8].value) if self.is_num(rsheet.row(x)[8].value) else rsheet.row(x)[8].value
			self.prod[sku]['min3'] = float(rsheet.row(x)[11].value) if self.is_num(rsheet.row(x)[11].value) else rsheet.row(x)[11].value
			self.prod[sku]['price3'] = float(rsheet.row(x)[10].value) if self.is_num(rsheet.row(x)[10].value) else rsheet.row(x)[10].value
