...
class Barn_Candles():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(rsheet.nrows):
			...
			if not self.is_num(rsheet.row(x)[11].value): #skip no price
				continue
			...
			self.prod[sku]['price1'] = float(rsheet.row(x)[11].value)/2 if "Kitchen" not in rsheet.row(x)[2].value and "Accessories" not in rsheet.row(x)[2].value else rsheet.row(x)[11].value * .75
			...
			self.prod[sku]['img400'] = "barn400"
			self.prod[sku]['img160'] = "barn160"
			self.prod[sku]['jpg400'] = rsheet.row(x)[18].value
			self.prod[sku]['jpg160'] = rsheet.row(x)[18].value
			self.prod[sku]['desc2'] = ""#rsheet.row(x)[12].value
			self.prod[sku]['opt'] = ""
			self.prod[sku]['img800'] = "barn800"
