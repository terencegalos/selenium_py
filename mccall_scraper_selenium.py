...
targetv.read_sitemap()			                                     #scraper mode get all items from sitemap
...
for item in targetv.links:
	...
	# if item == targetv.lastStop: # toggle flag if last stop reached
	# 	targetv.flag = True

	# if not targetv.flag: # skip until last stop
	# 	print "Skipping " + item
	# 	continue
		
	#if item == "http://www.packagingsource.com/store/p/468-Frosted-Euro-Totes-Queen.html":
	while True:
		try:
