...
set slist=start "" "K&K Interiors" "K&K Interiors 50 Off Closeouts" "K&K Jewelry"
set vlist=258 7960
...
set output=start "" "A_Cheerful_Giver" "Adams_and_Company" "Audreys_Your_Hearts_Delight" "Blossom_Bucket" "Delton_Products" "Designs_Combined" "Direct_International" "JanMichael's_Crafts" "K&K_Interiors" "KMI_International" "Kraft_Klub" "Pine_Creek_Four_Corners" "Raghu_-_Home_Collections" "The_Country_House_Collection" "VIP_International_Home_&_Garden" "Wholesale_Home_decor_(Harvest_Scents)"
...
set /p input= Enter vendor:
echo "Processing images for:" %input%

for %%a in (%input%) do ( 
   REM call python scraper_selenium.py %%a 
   ::call python image_checker.py %%a
   ::call python upload.py %%a
   REM call python image_download.py %%a
   call python img_resize.py %%a
   call python ftp.py %%a
)
