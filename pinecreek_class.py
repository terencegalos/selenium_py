...
class pinecreek(domainobject.domainobject):
    ...
    def save_info(self,item=None):
        db = gateway()
        try:
            db.name = self.driver.find_element_by_css_selector("#JS_PROD > div.content-container > div > div > div > div.row > div.col-sm-7 > h1").text.encode("utf-8")
        except:
            return
        db.sku = self.driver.find_element_by_css_selector("#JS_PROD > div.content-container > div > div > div > div.row > div.col-sm-7 > div.prod-code > em").text.encode("utf-8")
        try:
            db.cat = self.driver.find_element_by_css_selector("#main-content div:nth-child(5)").text.encode("utf-8")
        except:
            db.cat = ""
        db.desc = self.driver.find_element_by_css_selector("#JS_PROD > div.content-container > div > div > div > div.row > div.col-sm-7 > div.prod-desc").text.encode("utf-8")
        db.stock = ""
        db.sale = ""
        db.set = ""
        db.custom = ""
        ...
        db.seller = ""
        try:
            db.min1 = self.driver.find_element_by_css_selector("#JS_PROD > div.content-container > div > div > div > div.row > div.col-sm-7 > form > div.well.well-prod > div > div.col-sm-3.col-xs-4 > select > option:nth-child(1)").get_attribute("value")
        except:
            db.min1 = self.driver.find_element_by_name("Quantity").get_attribute("value")
        try:
            db.price1 = self.driver.find_element_by_css_selector("#price-value").text.encode("utf-8")
        except:
            db.price1 = ""
        db.min2 = ""
        db.price2 = ""
