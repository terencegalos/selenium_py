...
class Bethany_Lowe():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			self.prod[sku]['desc'] = ""
			# self.prod[sku]['stock'] = "In stock & shipping (checked "+str(datetime.datetime.today().strftime("%b %d"))+")" if rsheet.row(x)[6].value > 0 else ("Out of stock - order to ship after " + " ".join(rsheet.row(0)[5].value.split()[-2:]) if rsheet.row(x)[5].value > 0 else "Out of stock")
			self.prod[sku]['stock'] = ""
			self.prod[sku]['sale'] = ""
			self.prod[sku]['set'] = ""
			...
			self.prod[sku]['img400'] = "Bethany400"
			self.prod[sku]['img160'] = "Bethany160"
			self.prod[sku]['jpg400'] = ""
			self.prod[sku]['jpg160'] = ""
			self.prod[sku]['desc2'] = rsheet.row(x)[4].value
			self.prod[sku]['opt'] = ""
			self.prod[sku]['img800'] = "Bethany800"
