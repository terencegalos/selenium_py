...
class VIP_Home_Garden_Discount_Closeouts():
	prod = {}
	skus = None
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			if rsheet.row(x)[4].value < 4: #skip no stock
				continue
			...
			self.prod[sku]['size'] = rsheet.row(x)[3].value#str(rsheet.row(x)[5].value) + " x " + str(rsheet.row(x)[6].value) + " x " + str(rsheet.row(x)[7].value)
			self.prod[sku]['top'] = ""
			self.prod[sku]['min'] = 2#rsheet.row(x)[9].value
			self.prod[sku]['price1'] = rsheet.row(x)[5].value
			self.prod[sku]['min2'] = ""
			self.prod[sku]['price2'] = ""
			self.prod[sku]['min3'] = ""
			self.prod[sku]['price3'] = ""
			self.prod[sku]['multi'] = 2#rsheet.row(x)[9].value
