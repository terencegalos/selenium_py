...
class JanMichaels_Crafts():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			# print rsheet.row(x)
			if rsheet.row(x)[10].value == "": #skip no min
				continue
			...
			if "clearance" in rsheet.row(x)[9].value.lower(): # skip clearance
				continue
			...
			self.prod[sku]['cat'] = rsheet.row(x)[9].value +"|"+ rsheet.row(x)[2].value
			...
			# self.prod[sku]['size'] = "|".join([str(i) for i in re.findall('[Mm]easures\s\d{1,}[xX]\d{1,}\sinch'," ".join(rsheet.row(x)[14].value.split()),re.M)]) if not isinstance(rsheet.row(x)[14].value,float) else ""
			...
			if isinstance(rsheet.row(x)[7].value,float):
				self.prod[sku]['price1'] = round(float(rsheet.row(x)[7].value),2) if isinstance(rsheet.row(x)[7].value,float) else round(float(rsheet.row(x)[7].value.split("]")[1]),2)
			else:
				# print rsheet.row(x)[6].value.split("]")
				self.prod[sku]['price1'] = round(float(rsheet.row(x)[7].value.split("]")[1]),2)
