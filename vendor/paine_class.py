...
class paine(domainobject.domainobject):

    vendor = "Paine Products Inc"
    url = "https://paineproducts.com/"
    ...
    def aside_grabber(self):
        aside = [a.get_attribute("href") for a in self.driver.find_elements_by_css_selector("#menu-left-menu li > a")]
        print aside
        return aside

    def item_grabber(self):
        item = [a.get_attribute("href") for a in self.driver.find_elements_by_css_selector("li > ul > li > div > div > h4 > a")]
        print item
        return item
