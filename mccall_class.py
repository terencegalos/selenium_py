...
class mccall(domainobject.domainobject):
    ...
    def get_info(self,item=None):
        db = gateway()
        try:
            db.name = self.driver.find_element_by_css_selector("#ProductSection-product-template > div > div:nth-child(2) > div.product-single__meta > h1").text.encode("utf-8")
        except:
            return None
        # try:
			# db.sku = self.driver.find_element_by_css_selector("div[itemprop=sku]").text.encode("utf-8")
        jsonText = self.driver.find_element_by_css_selector("#ProductJson-product-template").get_attribute("innerHTML").encode("utf-8")
        print "\n"+jsonText+"\n"
        ...
        # except:
			# db.sku = item
        db.cat = ""#self.driver.find_element_by_css_selector("#product-attribute-specs-table > tbody > tr:nth-child(6) > td").text.encode("utf-8")
        db.desc = self.driver.find_element_by_css_selector("div.product-single__description.rte").text.encode("utf-8")
        db.stock = ""
        db.sale = ""
        db.set = ""
        db.custom = ""
        db.size = ""#self.driver.find_element_by_css_selector("#product-attribute-specs-table > tbody > tr:nth-child(5) > td").text.encode("utf-8")
        db.seller = ""
        db.min1 = 1
        db.price1 = 100
        db.min2 = ""
    ...
    def search_item(self,row):
        ...
        ActionChains(self.driver).move_to_element(self.driver.find_element_by_css_selector("#shopify-section-header > div:nth-child(3) > header > div > div.grid__item.medium-up--one-fifth.text-right.site-header__icons.site-header__icons--plus > div > button.btn--link.site-header__icon.site-header__search-toggle.js-drawer-open-top")).perform()
        self.driver.find_element_by_css_selector("#shopify-section-header > div:nth-child(3) > header > div > div.grid__item.medium-up--one-fifth.text-right.site-header__icons.site-header__icons--plus > div > button.btn--link.site-header__icon.site-header__search-toggle.js-drawer-open-top").click()
        self.time.sleep(1)
        ...
        self.time.sleep(1)
            ...
            #     continue
        try:
            item = [i.get_attribute("href") for i in self.driver.find_elements_by_css_selector("#MainContent > ul > li > div > a")]
            return item
        except:
            return None
