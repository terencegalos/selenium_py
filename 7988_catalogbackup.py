...
class Carson_Home_Accents():
	...
	def __init__(self,vendor,mode=2):
		...
		for x in range(1,rsheet.nrows):
			...
			if rsheet.row(x)[4].value == 0: # check price
				continue
			...
			self.prod[sku]['cat'] = rsheet.row(x)[7].value
			self.prod[sku]['desc'] = ""
			self.prod[sku]['stock'] = ""
			self.prod[sku]['sale'] = ""
			self.prod[sku]['set'] = ""
			self.prod[sku]['custom'] = 1 if "nd" in rsheet.row(x)[7].value.lower() else ""
			self.prod[sku]['size'] = ""
